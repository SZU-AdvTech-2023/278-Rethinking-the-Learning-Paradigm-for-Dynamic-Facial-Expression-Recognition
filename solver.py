...
class Solver(object):
    ...
    def train(self, epoch):
        ...
        for i, (images, target) in enumerate(self.train_dataloader):
            ...
            output_t = torch.zeros(self.args.batch_size, 7).cuda()
            ...
            loss_IAL = torch.mean(self.criterion(output_f - 9999 * (1 - target_mask - resmax_mask), target), dim=0)
            ##cross-entropy loss
            loss_CE = torch.mean(self.criterion(output_f, target), dim=0)

            ##(lamda=0.1)
            loss = loss_CE  + loss_IAL * 0.1
            pred = torch.argmax(output_f, 1).cpu().detach().numpy()
